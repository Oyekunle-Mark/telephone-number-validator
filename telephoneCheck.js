 ...
 * or reject the US phone number based on any combination of the formats
 *  provided above. The area code is required. If the country code is 
 * provided, you must confirm that the country code is 1. Return true if the 
 * string is a valid US phone number; otherwise return false. */

function telephoneCheck(str) {
  if (/^(1\s?)?((\([0-9]{3}\))|[0-9]{3})[\s\-]?[\0-9]{3}[\s\-]?[0-9]{4}$/.test(str))
    return true;
